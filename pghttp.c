/* PostgreSQL HTTP Extension - Cross-Platform (Windows/Linux) */
...
void _PG_init(void) {
#ifndef WIN32
    /* Initialize curl globally for Linux */
...
void _PG_fini(void) {
#ifndef WIN32
    /* Cleanup curl for Linux */
    curl_global_cleanup();
#endif
}

/* HTTP response structure (platform-independent) */
typedef struct {
    long status_code;
    char *content_type;
    char *body;
...
/* Perform HTTP request using WinHTTP (Windows) */
...
static size_t header_callback(char *buffer, size_t size, size_t nitems, void *userdata) {
    ...
    http_response_data *response = (http_response_data*)userdata;
    ...
    if (strncasecmp(buffer, "Content-Type:", 13) == 0) {
        ...
        if (end) {
            ...
            char *content_type = (char*)palloc(len + 1);
...
/* Perform HTTP request using libcurl (Linux) */
static http_response_data* perform_http_request(const char *method, const char *url,
    ...
    /* Add custom headers if provided */
    if (headers_str != NULL && strlen(headers_str) > 0) {
        /* Split headers by newline or semicolon */
        char *headers_copy = pstrdup(headers_str);
    ...
    /* Set User-Agent */
    curl_easy_setopt(curl, CURLOPT_USERAGENT, "pghttp/1.0");
...
/*
 * =============================================================================
 * PostgreSQL Function Implementations (Platform-Independent)
 * =============================================================================
 */
